...
Rails.application.configure do
  ...
  config.active_record.shard_resolver = ->(request) { request.respond_to?('subdomain') ? Tenant.find_by(subdomain: request.subdomain).shard  : Tenant.find_by(subdomain: '').shard }
end
